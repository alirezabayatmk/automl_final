...
def plot_seeds_trajectory(results_per_seed: dict) -> None:
    ...
    for (seed, budget_type), facade in results_per_seed.items():
        ...
        plt.step(X, Y, label=f"{budget_type} - Seed {seed}")  # Include seed in label

    plt.legend()
    plt.savefig('visualizations/trajectory_default_cs_seeds.png')

def plot_seeds_trajectory_from_log() -> None:
    plt.figure()
    plt.title("Trajectory")
    plt.xlabel("Wallclock time [s]")
    plt.ylabel("cost")  # cost
    ...
    paths = ["tmp/MF_default_img_size/0/intensifier.json"]


    for path in paths:
        logging.info(f"log from path {path}")
        ...
        label = path.split("/")[1]
        plt.plot(X, Y, label=f"{label}")  # Include seed in label
        plt.scatter(X, Y, marker="x")

    plt.legend()
    plt.show()
    plt.savefig('trajectory_seeds.png')


def train_mf_selection(cs: ConfigurationSpace) -> str:
    results_per_seed = {}
    fidelity_budgets = {'img_size': (8, 24), 'epochs': (5, 15)} # seed 0: 1. img_size 8 and 16 (epochs 10)
                                                                        # 2. epochs 5 and 10 (img 16)
    ...
    for seed in range(3):
        for fidelity, budgets in fidelity_budgets.items():
            ...
            scenario = Scenario(
                ...
                name=f"MFO({fidelity}_Seed{seed}_default_cs)"
            )
...
if __name__ == "__main__":
    #plot_seeds_trajectory_from_log()
    ...
    if args.mf_selection == "SPR_CORRELATION":
        ...
        fidelity_budgets = {'img_size': (8, 16), 'epochs': (3, 10)}
        ...
        best_fidelity = train_mf_selection(configspace)
